<template>
    <div id="search-address-container">
        ...
        <b-container class="px-0 main-container d-flex flex-column h-100" fluid>
            <b-card class="search-address-card h-100" body-class="d-flex flex-column h-100">
                ...
                <div class="search-result d-flex flex-column" v-show="addressList" v-else>
                    ...
                    <b-list-group>
                        <b-list-group-item v-for="(item, index) in addressList" :key="index" @click="chooseAddress(index)">
                            <h4>{{item.name}}</h4>
                            <p>{{item.address}}</p>
                        </b-list-group-item>
                    </b-list-group>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            chooseAddress(index) {
                this.saveAddressName(this.addressList[index].name)
                this.$router.go(-1)
            }
        }
    }
